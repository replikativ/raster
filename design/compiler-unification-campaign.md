...
| 3. Direct TypedSOAC and retirement | The measured 243-var corpus now has 217 flat TypedSOAC, 1 typed structured-control, 20 scalar and 5 explicit host-only rows, with no compatibility or error route. It emits 843 KernelBody artifacts and 1 verified compatibility artifact. That last artifact is the host-stateful `java.util.Random` dropout-mask generator; 27 programs record declines in total because rejected faster matrix candidates remain observable even when KernelBody wins. Generated matrix/input-fusion, reductions, ordinary counted stores, AD/SGD, ABM effects and numerical operators use retained typed programs. | Give dropout/random generation an explicit counter/state algebra before replacing the final emitter fallback; do not imply that arbitrary host RNG objects are device-pure. Audit the 20 scalar rows as host scalar APIs versus missing parallel structure rather than forcing them into device IR. Source-route coverage is not proof of competitive scheduling or vendor device performance. |
