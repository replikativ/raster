...
(deftest certified-plans-compose-before-allocation-through-one-node-identity
  (let [lowering (composed 16)
    ...
    (is (every? #(integer? (peek %)) (keys (:nodes plan)))
        "composed node identities are compact local ordinals")
    (is (every? #(integer? (peek %)) (keys (:values plan)))
        "source value identities remain in the certificate, not the hot plan maps")
    (is (every? #(integer? (peek (:id %))) (:instances plan))
        "composed instance identities avoid embedding complete source descriptors")
