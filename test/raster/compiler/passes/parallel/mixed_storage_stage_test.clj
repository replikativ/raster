...
(deftest mixed-storage-arithmetic-derives-width-from-typed-operands
  (let [source (-> (packed/packed-facts 1 1 1 4)
                   (assoc-in [:stages 1 :dtype] :float)
                   (assoc-in [:opts :stages 1 :dtype] :float))
        inferred (staged/lower source)
        typed (update source :body vary-meta assoc :raster.type/tag 'long)]
    (doseq [scheduled [inferred (staged/lower typed)]]
      (let [operations (tree-seq coll? seq (:operations (:body scheduled)))
          products (filter #(and (map? %) (= :* (:op %))) operations)]
        (is (= {:long 1 :float 2} (frequencies (map :result-type products)))
            "typed Byte loads prove one Long product before two Float scale products")))))
