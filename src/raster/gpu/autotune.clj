...
(defn schedule-neighbors
  "The moves coordinate-descent explores over an S6 Schedule: precision (two-valued), grf mode
   (two-valued), staging space. Bounded and hardware-agnostic; the feasibility gate + cost-fn prune
   the infeasible/slow. (split-k / launch-geometry knobs join as S6 grows them.)"
  [_desc]
  {:precision      (fn [_] [:f16-xmx :f32-scalar])
   [:grf :mode]    (fn [_] [:grf128 :grf256])
   [:stage :space] (fn [_] [:none :slm :l3])})
