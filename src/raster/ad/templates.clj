...
(def ^:private op-structures
  ...
  {;; --- (a) elementwise: diagonal Jacobian, frule = backward(tangent) ---
   ...
   'raster.dl.array-ops/pack-heads-strided {:class :linear :in #{0}}    ;; [x seq-len n-heads head-dim row-stride column-offset]
   'raster.dl.array-ops/unpack-heads     {:class :linear :in #{0}}      ;; [x seq-len n-heads head-dim]
   'raster.dl.array-ops/unpack-heads-strided {:class :linear :in #{0}}  ;; [x seq-len n-heads head-dim row-stride column-offset]
