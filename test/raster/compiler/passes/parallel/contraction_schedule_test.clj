...
(deftest applying-a-matrix-schedule-produces-a-target-neutral-kernel-body
  (let [contract-facts (facts/contraction-facts (matrix-form 128 128 128) :dtype :half)
    ...
    (is (= [:long :long] (mapv #(get-in % [:index :type]) [outer-loop inner-loop]))
        "K advancement and lookahead widen before addition")
    (is (every? #(instance? raster.compiler.ir.kernel_body.ForLoop %)
