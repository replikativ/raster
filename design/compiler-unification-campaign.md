...
| 3. Direct TypedSOAC and retirement | The measured 243-var corpus now has 217 flat TypedSOAC, 1 typed structured-control, 20 scalar and 5 explicit host-only rows, with no compatibility or error route. It emits 841 KernelBody artifacts and 3 verified compatibility artifacts. The latter are exactly dropout RNG plus the forward/backward chunked RMSNorm first stages; 29 programs record declines in total because rejected faster matrix candidates remain observable even when KernelBody wins. Generated matrix/input-fusion, reductions, ordinary counted stores, AD/SGD, ABM effects and numerical operators use retained typed programs. | Replace the three named emitter fallbacks before claiming full unification. Audit the 20 scalar rows as host scalar APIs versus missing parallel structure rather than forcing them into device IR. Source-route coverage is not proof of competitive scheduling or vendor device performance. |
