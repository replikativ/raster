...
| 3. Direct TypedSOAC and retirement | The measured 244-var corpus now has 217 flat TypedSOAC, 1 typed structured-control, 20 scalar and 6 explicit host-only rows, with no compatibility or error route. It emits 844 KernelBody artifacts and no compatibility artifacts. Explicit-seed SplitMix dropout is one portable typed map with no seed-array intermediate; only the convenience API that creates a host `java.util.Random` remains host-only. The 26 programs recording declines are rejected faster matrix candidates even though KernelBody wins. Generated matrix/input-fusion, reductions, ordinary counted stores, AD/SGD, ABM effects and numerical operators use retained typed programs. | Audit the 20 scalar rows as host scalar APIs versus missing parallel structure rather than forcing them into device IR. Source/emitter coverage is not proof of competitive scheduling or vendor device performance; matched accelerator execution and roofline evidence remain required. |
