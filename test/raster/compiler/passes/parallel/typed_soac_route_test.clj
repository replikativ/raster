...
(deftest scalar-shapes-and-stable-tensor-captures-route-a-nested-reduction
  (let [source
    ...
    (is (every? #(re-find #"for \(long [^ ]*fold_index_[0-9]+ = \(long\)\(0\);" %)
                kernel-sources)
        "KernelBody emits each typed Fold as explicit long-indexed scalar control")
  ...
  (let [source
    ...
    (testing "input fusion retains the exact typed source and rewritten stage graph"
      (let [source-graph (:source direct-refinement)
            candidate (kdispatch/alternative dispatch :xmx-direct-tile-inputs)
        ...
        (is (= [:contract] (mapv (comp last :id) (:nodes stages))))
